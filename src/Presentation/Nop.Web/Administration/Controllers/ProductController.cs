...
namespace Nop.Admin.Controllers
{
	public partial class ProductController : BaseAdminController
	{
		...
		public virtual ActionResult List()
		{
			...
			model.OrderByCreatedOn = true; //Order by createdOn by default
		...
		public virtual ActionResult ProductList(DataSourceRequest command, ProductListModel model)
		{
			...
			var products = _productService.SearchProducts(
				...
				showHidden: true,
				orderBy: model.OrderByCreatedOn ? ProductSortingEnum.CreatedOn : ProductSortingEnum.Position,
			...
			gridModel.Data = products.Select(x =>
			{
				...
				productModel.CreatedOn = x.CreatedOnUtc;
				//picture
		...
		[HttpPost, ActionName("List")]
		[FormValueRequired("exportxml-promua-all")]
		public virtual ActionResult ExportPromUaXmlAll(ProductListModel model)
		{
			if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
				return AccessDeniedView();
			...
			try
			{
				var xml = _exportManager.ExportProductsToXmlPromUa(products, Url);
				return new XmlDownloadResult(xml, "products.xml");
			}
			catch (Exception exc)
			{
