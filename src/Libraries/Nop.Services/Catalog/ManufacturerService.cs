...
namespace Nop.Services.Catalog
{
	...
	public partial class ManufacturerService : IManufacturerService
	{
		...
		private const string MANUFACTURERS_ALLBYCATEGORYID_KEY = "Nop.manufacturer.allbycategoryid-{0}-{1}-{2}";
		...
		public virtual IList<Manufacturer> GetManufacturersByCategoryId(int categoryId)
		{
			...
			return _cacheManager.Get(key, () =>
			{
				var query = from m in _manufacturerRepository.Table
							join pmm in _productManufacturerRepository.Table on m.Id equals pmm.ManufacturerId
							join pcm in _productCategoryRepository.Table on pmm.ProductId equals pcm.ProductId
