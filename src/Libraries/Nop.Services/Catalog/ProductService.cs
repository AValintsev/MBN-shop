	...
	public partial class ProductService : IProductService
	{
			...
			bool? overridePublished = null)
		{
			return SearchProducts(
				...
				pageSize,
				categoryIds,
				manufacturerId == 0 ? null : new List<int> { manufacturerId },
				storeId,
				vendorId,
				warehouseId,
				productType,
		...
		public IPagedList<Product> SearchProducts(
			out IList<int> filterableSpecificationAttributeOptionIds, bool loadFilterableSpecificationAttributeOptionIds = false,
			int pageIndex = 0,
			int pageSize = int.MaxValue,
			IList<int> categoryIds = null,
			...
			bool? overridePublished = null)
		{
			...
			if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
			{
				...
				pManufacturerIds.Value = commaSeparatedManufacturerIds;
			...
			else
			{
				...
				else if (orderBy == ProductSortingEnum.Position && manufacturerIds != null && manufacturerIds.Any())
				{
					...
					var firstManufacturerId = manufacturerIds[0];
					query =
						query.OrderBy(p => p.ProductManufacturers.FirstOrDefault(pm => pm.ManufacturerId == firstManufacturerId).DisplayOrder);
				}
